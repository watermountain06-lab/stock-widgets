...
- **종합 밸류에이션 card's two-box layout must follow the verdict, not be copied from NVDA literally** (2026-08-09, user request: "고평가 회사면 고평가 근거로 보이면 좋겠다"): NVDA's card reads "저평가 근거" / "주의 요소" because NVDA *is* 저평가 - a 고평가 ticker's left box must say "고평가 근거", not have "저평가" pasted in regardless of the actual verdict. Left-box header color reuses the same 5-tier palette immediately above; right box (주의 요소) stays a fixed neutral `var(--gold)` amber regardless of verdict, not the ticker's own `--accent2` (NVDA's current card still uses `--accent2` here - a pre-existing inconsistency, fix it when next touching NVDA's card too, not just on newly-migrated widgets). Full spec: `canonical_widget_spec.md`'s `#valuation` card-4 entry.
...
- **투자의견 요약**: stacked layout (`display:flex;flex-direction:column;gap:10px`, not a wrapped flex row), exactly four lines in order - 📅 다음 실적 / 투자의견 / 목표주가 / 결론. No separate valuation-verdict line here - that belongs on the 핵심 투자 논리 title tag instead (redundant with the 5개 분석 종합 card's ⚖️ row otherwise). **This one drifts silently** - MSFT (2026-08-09) had only 3 lines (투자의견 missing entirely) with the analyst count crammed into 결론 instead. Count the lines explicitly on every widget touched, don't assume 4 just because the card exists; when 투자의견 is missing, pull the count from the same 애널리스트 투자의견 분포 bar already on the 목표가 밴드 card (e.g. "Strong Buy (56명 중 매수 53)") rather than re-deriving it.
...
## Comparison-direction wording and scenario-card reproducibility - found by full-tab Codex audit, 2026-08-09

Two error classes that none of the existing scripted checks catch, since they're about prose logic rather than number consistency:

1. **"A는 B보다 낮다/높다" direction errors.** Writing several peer comparisons in one sentence ("NVDA 34.1x는 AVGO(50.9x)와 TSM(27.7x)보다 낮고...") is easy to get backwards for one clause when the peers straddle the subject value (34.1 < 50.9 but 34.1 > 27.7 - "보다 낮다" was wrongly applied to both). Caught on NVDA's PER explanation and MSFT's PBR explanation (MSFT 8.4x vs ADBE 8.7x) in the same review. When a sentence compares one value against 3+ peers with mixed directions, check each clause's inequality individually rather than trusting a single "낮다/높다" applied to the whole list.
2. **EV/EBITDA-style derived-number arithmetic drifting stale.** MSFT's EV/EBITDA explanation stated "EV 약 $2.86T÷EBITDA $193.8B" next to a displayed 19.0x multiple - $2.86T/$193.8B actually computes to 14.76x, not 19.0x. The EV figure was a leftover from an earlier price refresh that never got recomputed alongside the multiple itself. Any sourcing text that shows its arithmetic (X÷Y=Z) needs that arithmetic re-verified every time either the widget's price/market-cap or the multiple display value changes - a stale intermediate number is invisible until someone actually multiplies it out.
3. **Scenario dollar ranges must reverse-verify against the stated multiple/EPS, not just look plausible.** MSFT's Base scenario claimed "PER 27.9x가 자체 10년 중간값(32.6x) 방향으로 회복" but the paired dollar range ($450~520) reverse-computes to PER 25.1~29.0x against FY26 EPS $17.95 - the narrative text and the dollar figures were edited independently at different times and never cross-checked. Fix pattern used: tie the scenario to a real, already-established anchor in the file (here, the analyst consensus target price and its own implied PER) rather than inventing a new multiple assumption, then recompute the dollar range and the 목표가 밴드 gauge's segment widths/marker positions together (`target_band_gauge.py`) - changing one Bear/Base/Bull range without rerunning the gauge script leaves the visual marker in the wrong place even after the text is fixed.

None of `validate_widget.py`/`audit_widgets.py`/`audit_valuation_consistency.py` catch any of these three - they check internal number-vs-number consistency, not prose-logic-vs-number consistency. Worth a manual "read every comparison sentence and do the arithmetic" pass after any valuation-tab rewrite, not just after data refreshes.
