...
1. **멀티플 5단계 평가** — 5 `val-item`s, each with `stage-badge stage-N` (1–5), a `val-fill` gauge (green→gold→red 3-stop gradient, width = position on the low/mid/high scale), and numeric `val-labels` (never generic "낮음/적정/높음"). **The `mid`/"적정" reference value itself needs a stated source in the subtext, not just a plausible-looking round number** — see `stock-refresh/SKILL.md`'s "멀티플 5단계 평가" methodology section (2026-08-09): cite the ticker's own long-run historical median (GuruFocus) and a real peer average from the fleet's own already-verified widgets, adjusted for GAAP distortions and capital-structure incomparability (e.g. a foundry's PBR isn't comparable to a fabless peer's). An unsourced mid value is not just a documentation gap — 2 of NVDA's 4 unsourced values turned out to be materially wrong when checked.
